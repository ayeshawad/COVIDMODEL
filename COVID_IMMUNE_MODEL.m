...
dMPhi_I = p.a_I_MPhi*MPhi_R*(I+D)+p.p_MPhi_I_G*G_B^p.h_M_MPhi/(G_B^p.h_M_MPhi+p.eps_G_MPhi^p.h_M_MPhi)*M+p.p_MPhi_I_L*L_B/(L_B+p.eps_L_MPhi)*M*(p.eps_alpha_MPhi^(h_MPhi)/(p.eps_alpha_MPhi^(h_MPhi)+alpha_B^(h_MPhi)))-p.d_MPhi_I*MPhi_I-p.del_MPhi_D*MPhi_I*D-(1-MPhi_R/p.MPhi_max)*p.lam_MPhi*MPhi_I/(V+p.eps_V_MPhi);%-p.del_I_T*T*MPhi_I*100;%-p.del_MPhi_D*MPhi_I*T;    
...
dT = p.p_T_I*ylag2(3)/(1+L_B/p.eps_L_T)+p.p_T_F*F_B/(F_B+p.eps_F_T)*T-p.d_T*T -  (((p.del_T_K * T * K)/(K + p.eps_T_K))* (p.eps_K_L / (p.eps_K_L + L_B)));
