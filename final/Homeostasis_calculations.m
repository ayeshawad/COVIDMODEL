...
% <<TNF-ALPHA>>
p.alpha_U_star = p.alpha_U_0;
p.alpha_B_star = p.k_B_alpha*(p.T_star+p.K0+p.M_star)*p.A_alpha*p.alpha_U_star/(p.k_int_alpha+p.k_B_alpha*p.alpha_U_star+p.k_U_alpha);
p.alpha_B_0 = p.alpha_B_star;

% <<NATURAL KILLER CELLS>
%p.K_prod_star = (p.d_K*p.K0-p.p_K_A*p.alpha_B_0/(p.alpha_B_0+p.eps_K_A))*1/p.KR;  % homeostasis production rate for NK
p.K_prod_star = (p.d_K*p.K0-(p.p_K_A*p.alpha_B_star/(p.alpha_B_star+p.eps_K_A)))*(1/p.KR); 
end